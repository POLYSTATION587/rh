...
if($total_reg > 0){
	...
	for($i=0; $i < $total_reg; $i++){
		...
		$financiamento = $res[$i]['financiamento'];
		$numero_vaga = $res[$i]['numero_vaga'];
		$chave = $res[$i]['chave'];
		$complemento = $res[$i]['complemento'];
		$condicao_pgto = $res[$i]['condicao_pgto'];
...
<script type="text/javascript">
	...
	function editar(id, dono, corretor, tipo, cidade, bairro, status, condicao, area_total, area_construida, quartos, banheiros, suites, garagens, piscinas, ano, valor, iptu, condominio, endereco, comissao_imob, comissao_corretor, titulo, descricao, img_principal, img_banner, img_planta, video, validade, data_inicio, data_final, financiamento, chave, condicao_pgto, complemento, numero_vaga){
		...
		$('#video').val(video);
		$('#financiamento').val(financiamento).change();
		$('#condicao_pgto').val(condicao_pgto);
		$('#chave').val(chave);
		$('#complemento').val(complemento);
		$('#numero_vaga').val(numero_vaga);
	...
	function mostrar(id, doc, email, tel, dono, corretor, titulo, descricao, tipo, cidade, bairro, valor, ano, visitas, area_total, area_construida, quartos, banheiros, suites, garagens, piscinas, img_principal, img_planta, img_banner, endereco, status, condicao, video, iptu, condominio, comissao_imob, comissao_corretor, data_cadF, validade, data_inicio, data_final, financiamento, chave, condicao_pgto, complemento, numero_vaga){	
		...
		$('#financiamento_mostrar').text(financiamento);
		$('#numero_vaga_mostrar').text(numero_vaga);
		$('#chave_mostrar').text(chave);
		$('#complemento_mostrar').text(complemento);
		$('#condicao_pgto_mostrar').text(condicao_pgto);
